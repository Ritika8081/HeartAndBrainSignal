      ...
      <main className="flex-1 container mx-auto px-2 py-2">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 h-full">
          ...
          <div className="md:col-span-1 flex flex-col h-full gap-3">
            <div className={`flex-1 rounded-xl shadow-md p-3 border ${cardBg} flex flex-col items-center justify-center transition-colors duration-300`}>
              <div className={`p-3 rounded-full mb-2 ${iconBoxBg} transition-colors duration-300`}>
                <Box className={primaryAccent} strokeWidth={1.5} />
              </div>
            </div>

            <div className={`flex-1 rounded-xl shadow-md p-3 border ${cardBg} flex flex-col transition-colors duration-300`}>
          ...
          <div className="md:col-span-2 flex flex-col gap-3">
            <div className={`rounded-xl shadow-md p-3 border ${cardBg} flex flex-col items-center justify-center transition-colors duration-300`}>
                  ...
                  <div className="flex-1 h-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarDataCh0}>
                        ...
                        <PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={{ fill: axisColor, fontSize: 10 }} />
                        <Radar name="Ch0" dataKey="value" stroke={darkMode ? '#C29963' : '#A27C48'} fill={darkMode ? '#C29963' : '#A27C48'} fillOpacity={0.6} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                  ...
                  <div className="flex-1 h-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarDataCh1}>
                        ...
                        <PolarAngleAxis dataKey="subject" tick={{ fill: axisColor, fontSize: 10 }} />
                        <PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={{ fill: axisColor, fontSize: 10 }} />
                        <Radar name="Ch1" dataKey="value" stroke={darkMode ? '#548687' : '#2F6F6B'} fill={darkMode ? '#548687' : '#2F6F6B'} fillOpacity={0.6} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
            ...
            <div className={`flex-1 rounded-xl overflow-hidden p-2 transition-colors duration-300 ${darkMode ? 'bg-zinc-800/90' : 'bg-white'}`}>
              ...
              <WebglPlotCanvas data={eegData} channels={eegChannels} colors={channelColors} />
            </div>
          </div>
          ...
          <div className="md:col-span-2 flex flex-col gap-3">
            ...
            <div className={`flex-1 rounded-xl overflow-hidden p-2 transition-colors duration-300 ${darkMode ? 'bg-zinc-800/90' : 'bg-white'}`}>
              ...
              <WebglPlotCanvas data={ecgData} channels={ecgChannels} colors={channelColors} />
            </div>
          </div>
        </div>
      </main>
